...
;(function($, window, document, undefined) {
	...
	Tree.prototype = {
        ...
        setSelectedIndex: function (uuid)
        {
            this._setSelectedIndex(uuid);
        },
		...
		_destroy: function() {
			...
			if (this.initialized) {
                ...
                this.nodes.forEach(function (element){
                    if(element._nodes){
                        element.nodes = element._nodes;
                        delete element._nodes;
                    }
                });
		...
		_setSelectedNode: function(node) {
			...
			if (node === this.selectedNode) {
                this._triggerNodeSelectedEvent(this.selectedNode = {});
			}
			else {
        ...
        _setSelectedIndex: function(uuid) {
            for(var i = 0;i<this.tree.length;i++){
                var treeNode = this.tree[i];
                if(treeNode.uuid==uuid){
                    this._setSelectedNode(treeNode);
                    break;
                }
                if(treeNode.hasOwnProperty("nodes") && treeNode.nodes != null){
                    for(var j = 0;j<treeNode.nodes.length;j++){
                        this._setSelectedNode(treeNode.nodes[j]);
                        break;
                    }
                }
            }
